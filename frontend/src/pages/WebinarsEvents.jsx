...
const WebinarsEvents = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-center">
          <div className="bg-white rounded-lg p-1 shadow-md">
            <button
              ...
              className={`px-6 py-3 rounded-md font-semibold transition-all duration-200 ${activeTab === 'upcoming' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:text-blue-600'}`}
            >
              Upcoming Events ({upcomingWebinars.length})
            </button>
            <button
              onClick={() => setActiveTab('past')}
              className={`px-6 py-3 rounded-md font-semibold transition-all duration-200 ${activeTab === 'past' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:text-blue-600'}`}
            >
      ...
      {/* Webinars Content */}
      <section className="pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        {(activeTab === 'upcoming' ? upcomingWebinars : pastWebinars).map(webinar => (
          <div key={webinar.id} className="bg-white rounded-2xl shadow-xl overflow-hidden grid lg:grid-cols-3">
            {/* Speaker Section */}
            <div className="lg:col-span-1 bg-gradient-to-br from-blue-50 to-teal-50 p-8 text-center">
              <img src={webinar.speaker.image} alt={webinar.speaker.name} className="w-24 h-24 rounded-full mx-auto mb-4 object-cover border-4 border-white shadow-lg" />
              ...
              <div className="space-y-2">
                {webinar.speaker.credentials.map((cred, idx) => (
                  <div key={idx} className="bg-white px-3 py-1 rounded-full text-xs text-gray-700">{cred}</div>
                ))}
              </div>
            </div>
            ...
            <div className="lg:col-span-2 p-8 flex flex-col">
              <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  <span className={`px-3 py-1 rounded-full text-sm font-semibold ${webinar.isUpcoming ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                    {webinar.isUpcoming ? 'Upcoming' : 'Recorded'}
                  </span>
                  {webinar.isUpcoming && (
                    <button onClick={() => addToCalendar(webinar)} className="flex items-center space-x-2 text-blue-600 hover:text-blue-700 text-sm">
                      <Bell className="h-4 w-4" /> <span>Add to Calendar</span>
                    </button>
                  )}
                </div>
                ...
                {webinar.isUpcoming ? (
                  <>
                    <button className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors flex items-center justify-center space-x-2">
                      <ExternalLink className="h-4 w-4" /> <span>Register Now</span>
                    </button>
                  ...
                  <>
                    <button className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors flex items-center justify-center space-x-2">
                      <Play className="h-4 w-4" /> <span>Watch Recording</span>
                    </button>
                    <button className="border border-green-600 text-green-600 hover:bg-green-50 px-6 py-3 rounded-lg font-semibold transition-colors flex items-center justify-center space-x-2">
                      <Download className="h-4 w-4" /> <span>Download Materials</span>
                    </button>
                  </>
                )}
              </div>
      ...
      {/* Newsletter Signup */}
      <section className="py-16 bg-gradient-to-r from-blue-800 to-blue-900 text-center">
