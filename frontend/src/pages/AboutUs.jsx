...
const AboutUs = () => {
  ...
  // Scroll-to-mentors effect: runs on mount & whenever location.hash changes
  useEffect(() => {
    const hash = location.hash || "";
    if (hash === "#mentors" && mentorsRef.current) {
      // small delay helps if the component content is heavy and images need time to layout
      const id = setTimeout(() => {
        mentorsRef.current.scrollIntoView({ behavior: "smooth", block: "start" });
        // briefly highlight the section
        setHighlight(true);
        setTimeout(() => setHighlight(false), 2200);
      }, 120); // 120ms is small but helps ensure layout is ready
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="relative animate-slide-right">
              <img src="/images/About.png" alt="Oil and gas facility" className="rounded-2xl shadow-2xl transform hover:scale-105 transition-transform duration-700 ease-out" />
          ...
          <div className="grid md:grid-cols-3 gap-10">
            {mentors.map((mentor, i) => (
              <div key={i} className="bg-white rounded-2xl shadow-md hover:shadow-2xl transition p-6 text-center">
                <img src={mentor.image} alt={mentor.name} className="w-full h-64 object-cover rounded-lg mb-4" />
                ...
                <ul className="text-sm text-gray-700 space-y-1 mb-3">
                  {mentor.credentials.map((cred, idx) => <li key={idx}>• {cred}</li>)}
                </ul>
                <button className="text-gray-600 hover:text-teal-700 flex items-center justify-center gap-2">
            ...
            <div className="absolute inset-0 transform-style-3d animate-rotate-carousel flex justify-center items-center">
              {achievements.map((achievement, index) => (
                <div
                  ...
                  style={{ transform: `rotateY(${index * (360 / achievements.length)}deg) translateZ(300px)` }}
                >
