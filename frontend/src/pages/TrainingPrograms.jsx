...
// (For brevity in this snippet above I left the arrays indicated; in your actual file keep them as provided.)
...
import {
  Clock, Users, Award, BookOpen, Star, Play,
} from "lucide-react";
import AuthModal from "./AuthModal.jsx";
import { useAuth } from "../context/AuthContext.jsx";

// (DEFAULT_PROGRAMS and PROGRAMS_PART_2 arrays should be present above this component)
...
const TrainingPrograms = ({ programs }) => {
  ...
  const categories = useMemo(() => {
    // Count diploma vs non-diploma (self-paced)
    const diplomaCount = allPrograms.filter((p) => p.category === "diploma").length;
    const selfPacedCount = allPrograms.length - diplomaCount;

    return [
      { id: "all", name: "All Programs", count: allPrograms.length },
      { id: "self-paced", name: "Self Paced", count: selfPacedCount },
      { id: "diploma", name: "Diploma", count: diplomaCount },
    ];
  }, [allPrograms]);

  // --- CHANGED: filtering logic according to new categories
  const filteredPrograms = useMemo(() => {
    if (selectedCategory === "all") return allPrograms;
    if (selectedCategory === "self-paced") {
      // All programs except those with category === "diploma"
      return allPrograms.filter((program) => program.category !== "diploma");
    }
    // selectedCategory === "diploma"
    return allPrograms.filter((program) => program.category === "diploma");
  }, [selectedCategory, allPrograms]);

  // when user clicks the category button we change state AND update the URL
  const handleSetCategory = (catId) => {
    setSelectedCategory(catId);
    // reflect in URL so header links and direct links work consistently
    navigate(`/programs?category=${catId}`, { replace: false });
  };
