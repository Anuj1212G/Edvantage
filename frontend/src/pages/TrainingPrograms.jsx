...
const DEFAULT_PROGRAMS = [
  // ... (keep your DEFAULT_PROGRAMS exactly as before)
  {
    id: '1',
    title: 'Petroleum Engineering Excellence',
    category: 'diploma',
...
const TrainingPrograms = ({ programs }) => {
  ...
  const { user } = useAuth(); // <- use the hook exported by your unified context

  // use provided programs or fallback to default data
  const allPrograms = useMemo(() => (programs && programs.length ? programs : DEFAULT_PROGRAMS), [programs]);

  const [selectedCategory, setSelectedCategory] = useState('all');
  const [expandedCurriculum, setExpandedCurriculum] = useState(null);
  const [authOpen, setAuthOpen] = useState(false);
  const [redirectTo, setRedirectTo] = useState('/payment'); // default path
  ...
  const handleEnrollClick = (program) => {
    ...
    if (user) {
      // user logged in — navigate directly using router
      navigate(path);
      return;
    }

    // not logged in — open auth modal and pass redirectTo
    setAuthOpen(true);
    setRedirectTo(path);
  };
