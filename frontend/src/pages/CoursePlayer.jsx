...
export default function CoursePlayer() {
    ...
    if (isLoading) return <div className="flex items-center justify-center h-screen text-gray-500 text-lg font-medium">Loading course content...</div>;
    
    if (!isEnrolled) {
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-gray-50">
                <h2 className="text-3xl text-red-600 font-bold mb-4">Access Denied</h2>
                <p className="text-gray-700 mb-6 text-center max-w-md">You must purchase this course to view the content.</p>
                <button 
                    onClick={() => navigate(`/courses/${courseId}`)}
    ...
    if (!course || !currentVideo) return <div className="flex items-center justify-center h-screen text-gray-500 text-lg font-medium">Course structure error.</div>;

    return (
        <div className="flex h-screen bg-gray-50 overflow-hidden">
            {/* Left: Video Player */}
            <div className="flex-grow p-6 flex flex-col">
                <div className="relative w-full aspect-video bg-black rounded-xl shadow-lg overflow-hidden mb-6">
                    <video 
                        controls 
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                        <h2 className="text-white text-xl md:text-2xl font-semibold">{currentVideo.title}</h2>
                    </div>
                </div>
                <div className="p-4 bg-white rounded-xl shadow-md">
                    <h1 className="text-2xl md:text-3xl font-bold mb-2">{currentVideo.title}</h1>
                    <p className="text-gray-600 text-md md:text-lg">{currentVideo.description}</p>
                </div>
            </div>

            {/* Right: Lecture Navigation Sidebar */}
            <div className="w-96 bg-white p-6 overflow-y-auto border-l border-gray-200">
                <h3 className="text-2xl font-bold mb-6 sticky top-0 bg-white py-4 z-10">Course Modules</h3>
                <div className="space-y-3">
                    {course.videos.map((video, index) => (
                        <div 
                            key={video._id} 
                            onClick={() => setCurrentVideo(video)}
                            className={`p-4 rounded-xl cursor-pointer transition duration-300 transform hover:scale-[1.02] ${
                                currentVideo._id === video._id ? 'bg-blue-100 border-blue-500 border-2 shadow-md' : 'bg-gray-50 hover:bg-gray-100'
                            }`}
                        >
                            <p className="text-sm font-medium text-gray-500">Lecture {index + 1}</p>
                            <p className="text-md font-semibold text-gray-800">{video.title}</p>
                        </div>
                    ))}
                </div>
            </div>
